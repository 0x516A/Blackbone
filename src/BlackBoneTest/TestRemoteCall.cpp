...
namespace Testing
{
    ...
    public:
        ...
        {
            Assert::AreEqual( type, arg.type );
            Assert::AreEqual( size, arg.size );

            if constexpr (std::is_same_v<T, float>)
            {
                Assert::AreEqual( value, arg.imm_float_val );
            }
            else if constexpr(std::is_same_v<T, double>)
            {
                Assert::AreEqual( value, arg.imm_double_val );
            }
            else if constexpr (std::is_pointer_v<std::decay_t<T>>)
            {
                Assert::AreEqual( reinterpret_cast<uint64_t>(value), arg.imm_val64 );
            }
            else if (value != 0)
            {
                Assert::AreEqual( uint64_t( value ), arg.imm_val64 );
            }
        }
        ...
        TEST_METHOD( ConstructDirect )
        {
            ...
            Assert::AreEqual( size_t( 9 ), args.arguments.size() );
        ...
        TEST_METHOD( LocalCall )
        {
            ...
            RemoteFunction<decltype(&TestFn)> pFN( process, &TestFn );
            double d = 0.0;
            ...
            Assert::AreEqual( 3.0 + 2.0f, d, 0.001 );
            Assert::AreEqual( _cbuf, g_string );
            Assert::AreEqual( _wbuf, g_wstring );
        ...
        TEST_METHOD( NtQueryVirtualMemory )
        {
            ...
            Assert::IsTrue( Utils::FileExists( path ) );
            ...
            auto pFN = GetRemoteFunction<fnNtQueryVirtualMemory>( process, L"ntdll.dll", "NtQueryVirtualMemory" );
            Assert::IsTrue( pFN.valid() );
        ...
        TEST_METHOD( FileOP )
        {
            ...
            Assert::IsTrue( CreateFileWPtr.success() );
            Assert::IsTrue( WriteFilePtr.success() );
            Assert::IsTrue( CloseHandlePtr.success() );
            ...
            Assert::AreEqual( static_cast<DWORD>(sizeof( writeBuf )), bytes );
