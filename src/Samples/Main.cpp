...
int main( int /*argc*/, char* /*argv[]*/ )
{
    ...
    // Remote function calls
    {
        ...
        if (auto pMessageBoxW = GetRemoteFunction<decltype(&MessageBoxW)>( notepad, L"user32.dll", "MessageBoxW" ))
        {
        ...
        auto mainThread = notepad.threads().getMain();
        if (auto pIsGUIThread = GetRemoteFunction<decltype(&IsGUIThread)>( notepad, L"user32.dll", "IsGUIThread" ); pIsGUIThread && mainThread)
        {
        ...
        // Complex args
        if (auto pMultiByteToWideChar = GetRemoteFunction<decltype(&MultiByteToWideChar)>( notepad, L"kernel32.dll", "MultiByteToWideChar" ))
        {
